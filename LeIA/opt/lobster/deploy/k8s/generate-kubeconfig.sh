#!/usr/bin/env bash
# Generate a read-only kubeconfig for the Lobster ServiceAccount.
set -euo pipefail

namespace="saasphere-system"
secret_name="lobster-reader-token"
...
users:
  - name: lobster-reader
    user:
      token: ${token}
contexts:
  - name: lobster-reader@saasphere
    context:
      cluster: saasphere
      user: lobster-reader
current-context: lobster-reader@saasphere
EOF
)"
